...
def getAndSend(channel, address, port):
    ...
    browser = webdriver.Chrome(options=chrome_options)#options was chrome_options
    ...
    while True:
        for chat in browser.find_elements(By.CSS_SELECTOR,'yt-live-chat-text-message-renderer'):#find_elements was find_elements_by_css_selector
            usr = chat.find_element_by_css_selector("#author-name").get_attribute('innerHTML')#find_elements was find_elements_by_css_selector
            mex = chat.find_element_by_css_selector("#message").get_attribute('innerHTML')#find_elements was find_elements_by_css_selector
            usr = str(usr).split('<')[0]
